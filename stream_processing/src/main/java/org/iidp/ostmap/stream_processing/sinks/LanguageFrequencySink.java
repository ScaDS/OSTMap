 ...
 */
public class LanguageFrequencySink extends RichSinkFunction<Tuple3<String, String, Integer>> {

    private BatchWriter writerForFrequencies = null;

    private String tableFrequencies;
    private SinkConfiguration cfg;
    private Connector conn;

    private static final byte[] EMPTY_BYTES = new byte[0];

    private static Logger log = Logger.getLogger(LanguageFrequencySink.class);

    /**
     * empty constructor
     */
    public LanguageFrequencySink() {
    }
     ...
     * @param tableFrequencies      the table to write into
     * @throws AccumuloSecurityException
     * @throws AccumuloException
     * @throws TableNotFoundException
     * @throws TableExistsException
     */
    public void configure(SinkConfiguration sinkConfiguration, String tableFrequencies) throws AccumuloSecurityException, AccumuloException, TableNotFoundException, TableExistsException {
        log.info("configuring accumulo sink for " + tableFrequencies );
    ...
    public void invoke(Tuple3<String, String, Integer> value) throws Exception {
        ...
        if (writerForFrequencies == null) {
            writerForFrequencies = createBatchWriter(tableFrequencies);
        }

        //Write termData once per token/user
        Mutation mutationLangFreq = new Mutation(value._1().getBytes());
        mutationLangFreq.put(value._2().getBytes(), EMPTY_BYTES, (""+value._3()).getBytes());
        writerForFrequencies.addMutation(mutationLangFreq);
    }
}
